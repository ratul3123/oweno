    ...
    for (const e of expenses) {
      ...
      if (isPayer) {
        for (const s of e.splits) {
          // Skip user's own splits or already paid splits
          if (s.userId === user._id || s.paid) continue;

          // Add to amount to the user
          youAreOwed += s.amount;

          (balanceByUser[s.userId] ??= { owed: 0, owing: 0 }).owed += s.amount;
        }
      } else if (mySplit && !mySplit.paid) {
        // Someone else paind the user hasn't paid their split
        youOwe += mySplit.amount;

        // Add to amount the current user owes to the payer
      ...
      groups.map(async (group) => {
        ...
        expenses.forEach((expense) => {
          if (expense.paidByUserId === user._id) {
            ...
            expense.splits.forEach((split) => {
              // Add ammounts others owe to the user (excluding user's own split and paid splits)
        ...
        settlements.forEach((settlement) => {
          if (settlement.paidByUserId === user._id) {
            // User paid someone - increases user's balance
